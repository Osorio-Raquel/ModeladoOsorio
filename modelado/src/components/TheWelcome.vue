<template>
  <div class="generador-card">
    <h2 class="title">Generador Lineal Congruencial Óptimo</h2>

    <div class="form">
      <label>
        Período
        <input type="number" v-model.number="periodo" min="1" @input="validarNumero('periodo')"/>
      </label>

      <label>
        Semilla
        <input type="number" v-model.number="semilla" min="0" @input="validarNumero('semilla')"/>
      </label>
      ...
      <label>
        c (debe ser primo)
        <input type="number" v-model.number="c" min="2" @input="validarC"/>
      </label>
    ...
    <div v-if="resultadosBase.length" class="info-calc">
      <div><strong>g =</strong> {{ gComputed }} <span class="formula">(g = log₂(periodo))</span></div>
      <div><strong>m =</strong> {{ mComputed }} <span class="formula">(m = 2^g)</span></div>
      <div><strong>a =</strong> {{ aComputed }} <span class="formula">(a = 1 + 4*k)</span></div>
    </div>
    ...
    <div v-if="sugerenciasC.length" class="primos-sugeridos">
      <p>Lista de primos cercanos a c (clic para autocompletar):</p>
      <span 
        v-for="p in sugerenciasC" 
        :key="p" 
        class="primo"
        @click="seleccionarPrimo(p)"
      >
        {{ p }}
      </span>
    </div>
...
const error = ref("");
const sugerenciasC = ref([]);

// Fórmulas
const gComputed = computed(() => {
  if (!periodo.value || periodo.value <= 0) return null;
  return Math.ceil(Math.log(periodo.value) / Math.log(2));
});
const mComputed = computed(() => gComputed.value != null ? 2 ** gComputed.value : null);
const aComputed = computed(() => k.value != null ? 1 + 4 * k.value : null);

// Validación numérica inmediata
...
// Validar c primo
function validarC() {
  if (isNaN(c.value) || c.value < 2) {
    alert("c debe ser un número entero >= 2");
    c.value = null;
    return;
  }
  if (!esPrimo(c.value)) {
    alert("c debe ser primo");
    sugerenciasC.value = primosCercanos(c.value);
  } else {
    sugerenciasC.value = [];
  }
}

// Selecciona un primo de la lista
function seleccionarPrimo(primo) {
  c.value = primo;
  sugerenciasC.value = [];
}
...
function generar() {
  error.value = "";
  if (!periodo.value || !semilla.value || !k.value || !c.value) {
    error.value = "Complete todos los campos correctamente.";
    return;
  }
  if (!esPrimo(c.value)) {
    error.value = "c debe ser primo.";
    return;
  }
  ...
  for (let i = 0; i <= periodo.value; i++) {
    const operacion = `(${aComputed.value} * ${xiPrev} + ${c.value}) mod ${mComputed.value}`;
    const xi = (aComputed.value * xiPrev + c.value) % mComputed.value;
...
// Genera primos cercanos a c (±10)
function primosCercanos(n) {
  let primos = [];
  for (let i = n - 10; i <= n + 10; i++) if (i > 1 && esPrimo(i)) primos.push(i);
  return primos;
}
</script>
...
.form input { margin-top:0.4rem; padding:0.45rem 0.6rem; border-radius:8px; border:1px solid #e6e6e6; text-align:center; width:140px; }
...
.primos-sugeridos { margin-top:1rem; }
.primo { display:inline-block; background:#ffe6e6; color:#b30000; padding:0.3rem 0.6rem; margin:0.2rem; border-radius:6px; font-weight:600; cursor:pointer; transition:0.2s; }
.primo:hover { transform:scale(1.1); background:#ffcccc; }
</style>
